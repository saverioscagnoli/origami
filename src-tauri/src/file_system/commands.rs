use super::{dir, file, misc, platform_impl, DirEntry};
use crate::{
...
#[tauri::command(async)]
pub fn list_dir(app: AppHandle, dir: String, label: String) {
...
pub async fn create_entry(app: AppHandle, path: String, is_dir: bool, label: String) {
    let res = if is_dir {
        ...
        file::create_file(&path).await.map(|_| ())
    };
...
#[tauri::command(async)]
pub fn open_files(app: AppHandle, paths: Vec<String>, label: String) {
    let label_clone = label.clone();
...
pub async fn paste_entries(
    ...
    if cut {
        ...
        if size >= COPY_SIZE_THRESHOLD {
            ...
            paste_heavy(&app, paths, dest, label, win).await;
        } else {
            for (i, path) in paths.iter().enumerate() {
                let path = Path::new(&path);
                ...
                match path {
                    ...
                    Err(err) => {
                        Command::PasteEntries(None, Some(err.to_string()), i == paths.len() - 1)
                            .emit(&app, label.clone());
                    }
                }
            }
...
pub async fn paste_heavy(
    ...
    let on_entry_copied = |entry: DirEntry, is_finished| {
        println!("{:?}", entry.path);
        Command::PasteEntries(Some(entry), None, is_finished).emit(app, label.clone());
    };
    ...
    BackendEvent::CopyOver(start_time.elapsed().as_secs_f32())
        .emit_to(app, win.label().to_string());
...
#[tauri::command(async)]
pub fn star_entries(app: AppHandle, paths: Vec<String>, label: String) {
    let label_clone = label.clone();
    ...
    for (i, path) in paths.iter().enumerate() {
        ...
        match platform_impl::create_symlink(&path, &starred_path) {
            Ok(_) => {
                ...
                Command::StarEntries(Some(entry), None, is_last).emit(&app, label_clone.clone());
...
pub async fn unstar_entries(app: AppHandle, paths: Vec<String>, label: String) {
    ...
    for (i, path) in paths.iter().enumerate() {
        ...
        match res {
            Ok(_) => {
                Command::UnstarEntries(Some(entry), None, is_last).emit(&app, label_clone.clone());
